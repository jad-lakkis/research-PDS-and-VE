...
def run_episode(env: TileStreamingEnv, seed: int) -> dict:
    ...
    total_reward = 0.0
    coverages, stalls, n_enhanced, buffer_trace = [], [], [], [obs[0]]
    steps = 0
...
def main():
    ...
    print(f"{'ep':>3} {'steps':>6} {'total_r':>10} {'mean_r':>8} {'mean_cov':>9} "
          f"{'n_stalls':>9} {'max_stall':>10} {'mean_n_enh':>11} {'max_buf':>14} "
          f"{'neg_buf?':>9} {'nan/inf?':>9}")
    for i, r in enumerate(results):
        print(f"{i:>3} {r['steps']:>6} {r['total_reward']:>10.2f} {r['mean_reward']:>8.3f} "
              ...
              f"{r['mean_n_enhanced']:>11.1f} {r['max_buffer']:>14,.0f} "
    ...
    print(f"Mean coverage under a fully RANDOM policy: "
          ...
          f"(random ~32/64 tiles enhanced each step, so a non-trivial but well-below-1 "
          f"coverage is expected here - this is not yet a trained agent)")
